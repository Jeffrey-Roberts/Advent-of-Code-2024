...
public class Solution {
    public static List<List<Integer>> findSafeReports(String reports) {
        List<List<Integer>> reportsList = Arrays.stream(reports.split("\n"))
                .map(row -> Arrays.stream(row.split("\\s+"))
                        .map(Integer::parseInt)
                        .toList())
                .toList();

        reportsList = reportsList.stream().filter(report ->  isIncrementingOrDecrementingAtMostByN(report, 3)).toList();

        return reportsList;
    }

    private static boolean isIncrementingOrDecrementingAtMostByN(List<Integer> reports, int n) {
        boolean isIncrementing = true;
        boolean isDecrementing = true;

        for (int i = 0; i < reports.size() - 1; i++) {
            int currValue = reports.get(i);
            int nextValue = reports.get(i + 1);

            int incrementDifference = nextValue - currValue;
            int decrementDifference = currValue - nextValue;

            if (incrementDifference <= 0 || incrementDifference > n) {
                isIncrementing = false;
            }
            if (decrementDifference <= 0 || decrementDifference > n) {
