...
public class Solution {
    private record Result(boolean isIncrementingOrDecrementing, List<Integer> indexesFailed) {}

    public static List<List<Integer>> findSafeReports(String reports) {
        List<List<Integer>> reportsList = parseReports(reports);

        reportsList = reportsList.stream().filter(report ->
                isIncrementingAtMostByN(report, 3).isIncrementingOrDecrementing
                        || isDecrementingAtMostByN(report, 3).isIncrementingOrDecrementing).toList();

        return reportsList;
    }

    private static List<List<Integer>> parseReports(String reports) {
    ...
    public static List<List<Integer>> findSafeReportsWithProblemDampener(String reports) {
        List<List<Integer>> reportsList = parseReports(reports);

        reportsList = reportsList.stream().filter(report -> {
            Result isIncrementing = isIncrementingAtMostByN(report, 3);
            Result isDecrementing = isDecrementingAtMostByN(report, 3);

            if (isIncrementing.isIncrementingOrDecrementing || isDecrementing.isIncrementingOrDecrementing) return true;
            else {
                for (int index : isIncrementing.indexesFailed) {
                    List<Integer> alteredIncrementingList = removeElementByIndex(report, index);
                    isIncrementing = isIncrementingAtMostByN(alteredIncrementingList, 3);
                    if (isIncrementing.isIncrementingOrDecrementing) break;
                }


                for (int index : isDecrementing.indexesFailed) {
                    List<Integer> alteredDecrementingList = removeElementByIndex(report, index);
                    isDecrementing = isDecrementingAtMostByN(alteredDecrementingList, 3);
                    if (isDecrementing.isIncrementingOrDecrementing) break;
                }

                return isIncrementing.isIncrementingOrDecrementing || isDecrementing.isIncrementingOrDecrementing;
            }
    ...
    private static Result isIncrementingAtMostByN(List<Integer> reports, int n) {
        for (int i = 0; i < reports.size()-1; i++) {
            Integer currValue = reports.get(i);
            Integer nextValue = reports.get(i+1);

            int difference = nextValue - currValue;
            if (difference <= 0 || difference > n) return new Result(false, List.of(i, i+1));
        }
        return new Result(true, List.of());
    }

    private static Result isDecrementingAtMostByN(List<Integer> reports, int n) {
        for (int i = 0; i < reports.size()-1; i++) {
            Integer currValue = reports.get(i);
            Integer nextValue = reports.get(i+1);

            int difference = currValue - nextValue;
            if (difference <= 0 || difference > n) return new Result(false, List.of(i, i+1));
        }
        return new Result(true, List.of());
    }

    private static <T> List<T> removeElementByIndex(List<T> list, int index) {
    ...
    public static void main(String[] args) {
        String part1Input = """
                ...
                """;
        System.out.println(findSafeReports(part1Input).size());
        ...
        String part2Input = """
                ...
                56 57 60 63 65 67 68 70
                """;
        System.out.println(findSafeReportsWithProblemDampener(part2Input).size());
    }
}
