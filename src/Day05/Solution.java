...
public class Solution {
    public record IndexedValue(int value, int index) {}

    public static int sumValidPagesMiddleValue(String input) {
        String[] instructions = input.split("\n\n");
        Map<Integer, List<Integer>> orderingRules = parseInputToMap(instructions[0]);
        List<List<Integer>> pagesToProduce = parseInputToList(instructions[1]);
        List<List<IndexedValue>> pagesToProduceWithIndex = pagesToProduce.stream().map(Solution::parseToIndexedValues).toList();

        List<List<IndexedValue>> validPagesToProduce = pagesToProduceWithIndex.stream()
                .filter(pages -> isValidOrder(new ArrayList<>(pages.stream().map(elem -> elem.value).toList()), orderingRules)).toList();
        return  sumMiddlePages(validPagesToProduce);
    }

    public static int sumCorrectedInvalidPagesMiddleValue(String input) {
        String[] instructions = input.split("\n\n");
        Map<Integer, List<Integer>> orderingRules = parseInputToMap(instructions[0]);
        List<List<Integer>> pagesToProduce = parseInputToList(instructions[1]);
        List<List<IndexedValue>> pagesToProduceWithIndex = pagesToProduce.stream().map(Solution::parseToIndexedValues).toList();

        List<List<IndexedValue>> sortedPagesToProduceWithIndex = pagesToProduceWithIndex.stream().
                filter(pages -> !isValidOrder(new ArrayList<>(pages.stream().map(elem -> elem.value).toList()), orderingRules))
                .map(pages -> {
                        List<IndexedValue> sortedPages = new ArrayList<>(pages);
                        sortedPages.sort((element, target) -> {
                            if (orderingRules.containsKey(element.value)) {
                                List<Integer> orderValues = orderingRules.get(element.value);

                                if (orderValues.contains(target.value)) return target.index - element.index;
                            }
                            else if (orderingRules.containsKey(target.value)) {
                                List<Integer> orderValues = orderingRules.get(target.value);

                                if (orderValues.contains(element.value)) return  element.index - target.index;
                            }
        ...
        return sumMiddlePages(sortedPagesToProduceWithIndex);
    }

    private static boolean isValidOrder(List<Integer> list, Map<Integer, List<Integer>> orderingRules) {
        Map<Integer, Integer> indexes = new HashMap<>();
        indexes.put(list.getFirst(), 0);
        for (int index = 1; index < list.size(); index++) {
            int element = list.get(index);
            indexes.put(element, index);
            if (!orderingRules.containsKey(element)) continue;
            List<Integer> orderValues = orderingRules.get(element);

            for (int order : orderValues) {
                if (!indexes.containsKey(order)) continue;
                int position = indexes.get(order);
    ...
    private static int sumMiddlePages(List<List<IndexedValue>> pagesToProduce) {
        int total = 0;
        for (List<IndexedValue> pages : pagesToProduce) total += pages.get(pages.size() / 2).value;
        return total;
    }
    ...
    private static Map<Integer, List<Integer>> parseInputToMap(String input) {
        ...
        for (String entry : entries) {
            ...
            else map.put(key, new ArrayList<>(List.of(value)));
        }

        return map;
    }

    private static List<List<Integer>> parseInputToList(String input) {
    ...
    private static List<IndexedValue> parseToIndexedValues(List<Integer> list) {
        return IntStream.range(0, list.size()).mapToObj(i -> new IndexedValue(list.get(i), i)).toList();
    }
    ...
    public static void main() {
        String part1Input = """
                ...
                """;
        System.out.println(sumValidPagesMiddleValue(part1Input));
        ...
        String part2Input = """
                ...
                95,73,93,99,19,43,64
                """;
        System.out.println(sumCorrectedInvalidPagesMiddleValue(part2Input));
    }
}
