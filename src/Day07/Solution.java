...
public class Solution {
    ...
    private static List<Calibration> parseInput(String input) {
        List<Calibration> calibrations = new ArrayList<>();
        for (String row : input.split("\n")) {
            String[] keyValuePair = row.split(":");
            BigInteger key = new BigInteger(keyValuePair[0]);
            List<BigInteger> values = Arrays.stream(keyValuePair[1].trim().split("\\s+")).map(BigInteger::new).toList();
            calibrations.add(new Calibration(key, values));
        }
        return calibrations;
    }
